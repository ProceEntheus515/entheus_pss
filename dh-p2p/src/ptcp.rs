...
impl PTCPPacket {
    pub fn parse(data: &[u8]) -> PTCPPacket {
...
impl PTCP for UdpSocket {
    ...
    async fn ptcp_try_read(&self) -> Result<PTCPPacket, String> {
        let mut buf = [0u8; 4096];
        let result = time::timeout(time::Duration::from_secs(30), self.recv(&mut buf)).await;
        ...
        match result {
            ...
            Ok(Ok(n)) => Err(format!("Paquete demasiado corto: {} bytes", n)),
            Ok(Err(e)) => Err(format!("Error de red: {}", e)),
            Err(_) => Err("Timeout de lectura (30s sin datos)".to_string()),
        }
    }
}
